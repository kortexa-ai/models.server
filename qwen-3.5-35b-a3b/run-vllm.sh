...
# Qwen 3.5 35B-A3B on bare-metal vLLM with online FP8 quantization
# Architecture: Qwen3_5MoeForConditionalGeneration (MoE, 35B total / 3B active)
# Best config on DGX Spark: FP8 weights + TRITON_ATTN + FP8 KV cache → ~33-42 tok/s
...
echo "Starting Qwen 3.5 35B-A3B (FP8) via bare-metal vLLM"
echo "Model: ${MODEL}"
echo "Port: ${PORT}"
...
# Skip --quantization for auto (lets vLLM detect from checkpoint, e.g. GPTQ)
