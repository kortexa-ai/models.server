...
### Option 1: vLLM via NVIDIA Docker Container
...
### Option 2: llama-server (llama.cpp) with GGUF
...
- Supports Qwen3.5 models via GGUF quantizations (unsloth repos)
- OpenAI-compatible API with `--jinja` flag
- Lightweight, no Python dependency hell
- Performance: ~527 tok/s prompt, ~77 tok/s generation (Qwen3.5-2B Q8_0)

### When to use which
- **vLLM container:** Multiple concurrent users, need continuous batching, supported model architectures
- **llama-server:** Qwen3.5 (not yet in vLLM), flexible GGUF quantizations, lighter footprint

## What Doesn't Work Natively (as of March 2026)
...
- **Use the NVIDIA Docker container instead** (see above)
...
| qwen-3.5-27b | 2026 | unsloth/Qwen3.5-27B-GGUF | UD-Q8_K_XL |
