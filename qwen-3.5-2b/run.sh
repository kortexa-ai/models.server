...
MODEL_MLX="mlx-community/Qwen3.5-2B-MLX-4bit"
PORT="${PORT:-2030}"
HOST="${HOST:-0.0.0.0}"
...
if [[ "$OS" == "Darwin" ]]; then
    ...
    # Memory fraction: 0.1 = ~13GB
    GPU_MEM_FRAC="${GPU_MEM_FRAC:-0.1}"
    MODEL_HF="Qwen/Qwen3.5-2B-Instruct-AutoRound-int4-sym"
    ...
    llama-server -hf "unsloth/Qwen3.5-2B-GGUF:${QUANT}" --alias qwen-3.5-2b --host "$HOST" --port "$PORT" \
