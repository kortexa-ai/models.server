...
# Gemma 4 31B (dense) on bare-metal vLLM with online FP8 quantization
# Architecture: Dense, 31B params all active, up to 256K native context
# 48 layers, 16 kv_heads, 256 head_dim (full attention layers use 4 global kv_heads, 512 head_dim)
# Mixed sliding (1024 window) + full attention like the 26B-A4B
# KV math (fp8, 4 reqs @ 131K):
#   sliding: 2 * 40 * 16 * 256 * 1 * 1024  * 4 ≈ 1.3 GiB
#   full:    2 *  8 *  4 * 512 * 1 * 131072 * 4 ≈ 4.3 GiB  → ~6 GiB total, budget 8 GiB
...
echo "Starting Gemma 4 31B (FP8) via bare-metal vLLM"
echo "Model: ${MODEL}"
echo "Port: ${PORT}"
